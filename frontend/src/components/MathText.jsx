...
function MathText({ text, className = "", isTamil = false, department = "" }) {
  if (!text) return null;

  // Determine if Tamil department font should be applied
  const isTamilDept = isTamil || (department && (
    String(department).toLowerCase().includes('tamil') ||
    String(department).toUpperCase() === 'TAM'
  ));

  const fontClass = isTamilDept ? "tamil-department-text" : "";
  const combinedClass = `${fontClass} ${className}`.trim();
